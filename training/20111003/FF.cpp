...
//return;
	if (!isPrime(x)) {
		for (int i = 0; i < realm && x > 1 && x / fac[i] >= fac[i]; ++i) {
			if(fir.find(x)!=fir.end())break;
			int cnt = 0, now = fac[i] ,xx=x; 
		...
		while(x!=1&&fir.find(x)!=fir.end()){
			//cout << x << ' ' << fac[fir[x]] << endl;
			int cnt=0,now=fac[fir[x]],nn=fir[x];
			while(x%now==0)cnt++,x/=now;
			change(nn,cnt,mark);
		}
	}
